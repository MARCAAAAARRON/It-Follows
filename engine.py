...
class GameEngine(EventDispatcher):
    ...
    def make_open_rect(self):
        ...
        # Prefer reachable tiles so items aren't enclosed by walls
        if self.reachable:
            cx, cy = random.choice(self.reachable)
    ...
    def compute_reachable(self):
        # BFS from start (1,1) to find open tiles reachable by the player
        self.reachable = []
        if not self.maze or not self.rows or not self.cols:
            return
        start = (1, 1)
        if self.maze[start[1]][start[0]] == 1:
            return
        ...
        while q:
            ...
            for dx, dy in ((1,0), (-1,0), (0,1), (0,-1)):
                nx, ny = x + dx, y + dy
                if 0 <= nx < cols and 0 <= ny < rows and not seen[ny][nx] and self.maze[ny][nx] == 0:
                    seen[ny][nx] = True
                    q.append((nx, ny))
    ...
    def reset_level(self):
        ...
        # Recompute reachable tiles for spawning
